/**
 * maritime-pdf.ts — Generateur PDF Frais Maritimes
 * Couleur accent : #0284C7 (Sky blue)
...
export interface FraisMaritimeData {
  numero: string
  date: string
  client: {
    nom: string
...
export function generateMaritimePDF(data: FraisMaritimeData): ReturnType<typeof createDoc> {
  const doc = createDoc()
  const lang = data.lang || 'fr'
  const accent = PDF_COLORS.maritime

  const titlePrefix = lang === 'en' ? 'Shipping costs' : 'Frais maritimes'
