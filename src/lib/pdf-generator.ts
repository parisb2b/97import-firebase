...
export type PdfDocumentType =
  | 'DV' | 'DV_VIP' | 'DC' | 'FAC' | 'FA' | 'AF' | 'NC' | 'FL' | 'AI' | 'BL';
...
const COLORS = {
  ...
  dark: '#334155',       title: '#2C3E50',
  text: '#495057',       textStrong: '#1F2937',
  mediumGray: '#94A3B8', softGray: '#9CA3AF',
  lightGray: '#CBD5E1',  veryLightGray: '#F8FAFC',
...
const EMETTEUR_LINES = [
  'LUXENT LIMITED', '2ND FLOOR COLLEGE HOUSE', '17 KING EDWARDS ROAD',
  'RUISLIP HA4 7AE, LONDON UK', 'N° 14852122', 'luxent@ltd-uk.eu',
  'TEL & WHATSAPP : +33 6 20 60 74 48', 'WeChat : +86 135 66 27 19 02'
];
...
const CLIENT_FACT_LINES = [
  'Michel Chen', '12 Rue Port', '97200 Fort-de-France',
  'Martinique (MQ)', 'mc@sasfr.com'
];

const CLIENT_LIV_LINES = [
  'Michel Chen', '12 Rue Port', '97200 Fort-de-France', 'Martinique (MQ)'
];
...
function setText(doc: jsPDF, hex: string) { doc.setTextColor(...hexToRgb(hex)); }
...
function getDocConfig(type: PdfDocumentType) {
  const configs: Record<string, { title: string; color: string; bg: string; secondaryTitle: string; secondaryColor: string; secondaryBg: string }> = {
...
function drawHeader(doc: jsPDF, data: PdfData): void {
  ...
  doc.roundedRect(15, 15, 18, 18, 4, 4, 'F');
  ...
  doc.setFontSize(9.5);
  setText(doc, COLORS.softGray);
  const refText = data.ref ? ` · ${data.ref}` : '';
  doc.text(`N° ${data.number}${refText}`, 38, 31);
  setFill(doc, config.bg);
  doc.roundedRect(160, 15, 35, 12, 6, 6, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(6.8);
  setText(doc, COLORS.mediumGray);
  doc.text("DATE D'EMISSION", 177.5, 19, { align: 'center' });
  doc.setFontSize(8.8);
  setText(doc, config.color);
  doc.text(data.date, 177.5, 24, { align: 'center' });
}

// ============ DRAW: INFO BOX ============
function drawInfoBox(doc: jsPDF, opts: {
  x: number; y: number; w: number; h: number; title: string; lines: string[];
  borderColor: string; bgColor?: string; titleColor?: string; thickLeft?: boolean; center?: boolean;
}): void {
  const { x, y, w, h, title, lines, borderColor, bgColor = COLORS.white, titleColor = borderColor, thickLeft = false, center = false } = opts;
  setFill(doc, bgColor);
  setDraw(doc, thickLeft ? COLORS.border : borderColor);
  doc.setLineWidth(0.25);
  doc.roundedRect(x, y, w, h, 3, 3, 'FD');
  if (thickLeft) { setDraw(doc, borderColor); doc.setLineWidth(1.3); doc.line(x, y + 2.5, x, y + h - 2.5); }
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7.5);
  ...
  lines.forEach((line, i) => {
    ...
    doc.text(split, center ? x + w / 2 : x + 5, cy, { align: center ? 'center' : 'left' });
    cy += split.length * 4.2;
  });
}
...
function drawCommonBlocks(doc: jsPDF, data: PdfData): void {
  ...
  drawInfoBox(doc, { x: 107, y: 45, w: 88, h: 58, title: config.secondaryTitle, lines: getSecondaryLines(data), borderColor: config.secondaryColor, bgColor: config.secondaryBg, center: !['DV','DV_VIP','FAC','FA'].includes(data.type) });
  if (!['NC','AI'].includes(data.type)) {
...
function drawNote(doc: jsPDF, data: PdfData): number {
  ...
  const color = data.type === 'AI' ? COLORS.orange : COLORS.blue;
  setFill(doc, data.type === 'AI' ? COLORS.orangeBg : COLORS.blueBg);
  doc.roundedRect(15, 108, 180, 13, 2, 2, 'F');
  setDraw(doc, color); doc.setLineWidth(1.2); doc.line(15, 108, 15, 121);
...
function getTableDefinition(data: PdfData): { head: string[][]; body: any[][]; columnStyles: UserOptions['columnStyles'] } {
  switch (data.type) {
    case 'DV': case 'DV_VIP': case 'DC': case 'FA':
      return { head: [['RÉF.','QT','DÉSIGNATION','PRIX PUBLIC','PRIX VIP','TOTAL HT']], body: data.items.map(i => [i.ref||'—',String(i.qt||''),i.desc,euro(i.publicPrice),euro(i.vipPrice),euro(i.total)]), columnStyles: { 0:{cellWidth:22},1:{cellWidth:12,halign:'center'},2:{cellWidth:62},3:{cellWidth:28,halign:'right'},4:{cellWidth:28,halign:'right'},5:{cellWidth:28,halign:'right'} } };
    case 'FAC': case 'AF': case 'FL':
      return { head: [['RÉF.','QT','DÉSIGNATION','MONTANT HT']], body: data.items.map(i => [i.ref||'—',String(i.qt||''),i.desc,euro(i.amount??i.total)]), columnStyles: { 0:{cellWidth:24},1:{cellWidth:14,halign:'center'},2:{cellWidth:94},3:{cellWidth:48,halign:'right'} } };
    case 'NC':
      return { head: [['RÉF.','QT','DÉSIGNATION','PRIX VENTE FINAL','PRIX PLANCHER','COMM. UNIT.','COMM. TOTALE']], body: data.items.map(i => [i.ref||'—',String(i.qt||''),i.desc,euro(i.vipPrice??i.unitPrice),euro(i.floorPrice),euro(i.commissionUnit),euro(i.total??i.amount)]), columnStyles: { 0:{cellWidth:20},1:{cellWidth:10,halign:'center'},2:{cellWidth:45},3:{cellWidth:28,halign:'right'},4:{cellWidth:27,halign:'right'},5:{cellWidth:25,halign:'right'},6:{cellWidth:25,halign:'right'} } };
    case 'AI':
      return { head: [['RÉF.','QT','DÉSIGNATION','PRIX ACHAT','TOTAL']], body: data.items.map(i => [i.ref||'—',String(i.qt||''),i.desc,euro(i.unitPrice??i.amount),euro(i.total)]), columnStyles: { 0:{cellWidth:24},1:{cellWidth:14,halign:'center'},2:{cellWidth:82},3:{cellWidth:30,halign:'right'},4:{cellWidth:30,halign:'right'} } };
    case 'BL':
      return { head: [['RÉF.','QT','DÉSIGNATION']], body: data.items.map(i => [i.ref||'—',String(i.qt||''),i.desc]), columnStyles: { 0:{cellWidth:28},1:{cellWidth:18,halign:'center'},2:{cellWidth:134} } };
  }
}
...
function drawTable(doc: jsPDF, data: PdfData, startY: number): number {
  ...
  autoTable(doc, {
    startY, head: table.head, body: table.body, theme: 'grid',
    ...
    didParseCell: (hook) => {
      ...
      if (['DV','DV_VIP','DC','FA'].includes(type)) {
        if (col === 3) hook.cell.styles.textColor = hexToRgb(COLORS.lightGray);
        if (col === 4) { hook.cell.styles.textColor = hexToRgb(COLORS.violet); hook.cell.styles.fontStyle = 'bold'; }
        if (col === 5) { hook.cell.styles.textColor = hexToRgb(COLORS.textStrong); hook.cell.styles.fontStyle = 'bold'; }
      }
      if (['FAC','AF','FL','AI','NC'].includes(type)) {
        if (col === hook.table.columns.length - 1) hook.cell.styles.fontStyle = 'bold';
      }
    },
    didDrawCell: (hook: CellHookData) => {
      if (hook.section === 'body' && ['DV','DV_VIP','DC','FA'].includes(data.type) && hook.column.index === 3) {
        const { cell } = hook;
        setDraw(doc, COLORS.lightGray);
...
function drawPaidStamp(doc: jsPDF): void {
  ...
  setText(doc, COLORS.red); setDraw(doc, COLORS.red);
...
function drawTotals(doc: jsPDF, data: PdfData, y: number): void {
  ...
  function line(label: string, value: string, strong = false, color = COLORS.textStrong) {
    ...
    doc.text(label, x, cy); doc.text(value, x + w, cy, { align: 'right' }); cy += strong ? 7 : 5;
  }
  if (data.type === 'NC') { line('Total Prix Vente :', euro(data.totalSale)); line('Total Prix Plancher :', `- ${euro(data.totalFloor).replace('-','')}`); line('Marge :', `${data.marginPercent??0}%`); drawGrandTotal(doc, cy, 'COMMISSION TOTALE :', euro(data.totalCommission), config.color, config.bg); return; }
  if (data.type === 'AF') { drawGrandTotal(doc, cy, 'TOTAL AVOIR :', `- ${euro(Math.abs(data.totalCredit||0))}`, COLORS.red, COLORS.redBg); return; }
  if (data.type === 'FL') { drawGrandTotal(doc, cy, 'TOTAL LOGISTIQUE :', euro(data.totalLogistics??data.totalTTC), config.color, config.bg); return; }
  if (data.type === 'AI') { drawGrandTotal(doc, cy, 'TOTAL ACHAT :', euro(data.totalPurchase??data.totalTTC), config.color, config.bg); return; }
  if (data.type === 'DC') { drawGrandTotal(doc, cy, 'TOTAL COMMANDE :', euro(data.totalOrder??data.totalTTC), config.color, config.bg); return; }
  line('Total HT :', euro(data.totalHT)); line('TVA (0%) :', euro(0)); drawGrandTotal(doc, cy, 'TOTAL TTC :', euro(data.totalTTC), config.color, config.bg);
}
...
function drawGrandTotal(doc: jsPDF, y: number, label: string, value: string, color: string, bg: string): void {
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(9.5); setText(doc, color);
  doc.text(label, 130, y + 7); doc.setFontSize(11.5); doc.text(value, 192, y + 7, { align: 'right' });
}
...
function renderDocument(doc: jsPDF, data: PdfData): void {
  ...
  if (['NC','AI'].includes(data.type)) { drawNote(doc, data); drawCommonBlocks(doc, data); }
  else drawCommonBlocks(doc, data);
...
// Conservés pour ne pas casser les 13 fichiers importateurs existants.
// src/admin/pages/ListeDevis.tsx, DetailDevis.tsx, etc.
...
export function generateFactureFinale(quote: any, numero: string, _emetteur?: any): jsPDF {
  ...
  // Récupérer les paiements (acomptes encaissés)
...
export function generateNoteCommission(note: any, _emetteur?: any): jsPDF {
  ...
  // Supporte les deux formats : lignes plates ou groupées par devis
